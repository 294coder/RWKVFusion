...
def main_run_captioning():
    parser = argparse.ArgumentParser(
        description="Run image captioning with Florence-2 model"
    )
    parser.add_argument(
        "--img_dir",
        type=str,
    ...
    parser.add_argument(
        "--dataset", type=str, required=True, help="Dataset name for output file naming"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="results/captions",
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Interval for flushing results to file",
    )
    ...
    for i, img_path in enumerate(img_lst, 1):
        img = Image.open(img_path.as_posix())
        img = img.convert("RGB")
        caption = process_image(
            mode_dropdown=IMAGE_CAPTION, image_input=img, text_input=""
        )
        print(f"[{i}/{len(img_lst)}] - {img_path.stem}: {caption}\n")

        img_name = img_path.stem
        caption_df.loc[img_name, "caption"] = caption

        if caption_df.shape[0] % args.flush_interval == 0 or caption_df.shape[0] == len(
            img_lst
        ):
            output_path = os.path.join(args.output_dir, f"caption_{args.dataset}.csv")
            caption_df.to_csv(output_path, mode="w+")
            print("flush file\n\n")


if __name__ == "__main__":
    """python run_captioning.py --img_dir /path/to/images --dataset my_dataset"""
    main_run_captioning()
