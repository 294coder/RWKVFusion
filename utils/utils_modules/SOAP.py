...
if __name__ == "__main__":
    ...
    net = nn.Conv2d(10, 10, 3, 1, 1).cuda()
    optimizer = SOAP(net.parameters(), lr=0.01)
    net, optimizer = acc.prepare(net, optimizer)

    for _ in range(10):
        net(torch.randn(1, 10, 64, 64).cuda()).mean().backward()
        optimizer.step()
    print(net.weight)
