...
class UNet5(nn.Module):
    def __init__(self, input_channel=1):
        ...
        self.up_sample5 = nn.Sequential(
            nn.ConvTranspose2d(1024, 512, 4, 2, 1, bias=False), nn.ReLU()
        )
        self.up4 = ConvBlock(1024, 512)
        self.up_sample4 = nn.Sequential(
            nn.ConvTranspose2d(512, 256, 4, 2, 1, bias=False), nn.ReLU()
        )
        self.up3 = ConvBlock(512, 256)
        self.up_sample3 = nn.Sequential(
            nn.ConvTranspose2d(256, 128, 4, 2, 1, bias=False), nn.ReLU()
        )
        self.up2 = ConvBlock(256, 128)
        self.up_sample2 = nn.Sequential(
            nn.ConvTranspose2d(128, 64, 4, 2, 1, bias=False), nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        if sz[-1] % 32 != 0 or sz[-2] % 32 != 0:
            out = F.interpolate(out, size=sz[-2:], mode="bilinear", align_corners=False)
