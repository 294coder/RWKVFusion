...
def clip_value_(
    ...
    if accelerator is not None:
        if accelerator.sync_gradients:
            accelerator.clip_grad_value_(network.parameters(), max_value)
    else:
...
def step_loss_backward(
    ...
    mixed_precision = fp16 or mix_precision

    optim.zero_grad()
    # vanilla pytorch mix-precision
    if mixed_precision and accelerator is None:
    ...
    # accelerate mix-precision
        ...
        if max_norm is not None:
            clip_norm_(max_norm, network, fp_scaler, optim, accelerator)
        elif max_value is not None:
            clip_value_(max_value, network, fp_scaler, optim, accelerator)

        if not grad_accum and accelerator is not None:
            optim.step()
