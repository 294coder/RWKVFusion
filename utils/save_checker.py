...
class BestMetricSaveChecker:
    ...
    def __init__(
        ...
        *,
        avg_metrics_weights: "dict[str, float] | None" = None,
        verbose: bool = True,
    ):
        self.state = accelerate.state.PartialState()
        self.verbose = verbose and self.state.is_main_process

        # overide the avg_metrics_weights if it is None
        self.metric_name: "str | list[str] | dict[str, float]"
        if is_none(avg_metrics_weights):
            if isinstance(metric_name, (list, tuple)):
                self.metric_name = metric_name
                avg_metrics_weights = {m: 1.0 for m in metric_name}
                ...
                self.metric_name = metric_name
                avg_metrics_weights = {metric_name: 1.0}
            else:
                raise ValueError(
                    f"@metric_name should be a str or a list of str or a dict of str->float but got {metric_name}"
                )
        else:
            self.metric_name = metric_name
        ...
        if is_none(metric_name):
            logger.warning(
                f"No metric name provided, and SaveChecker will alway return True"
            )
        ...
        self.avg_metrics = (
            isinstance(self.metric_name, (tuple, list))
            and avg_metrics_weights is not None
        )

        if self.state.is_main_process:
            logger.info(
                f"SaveChecker initialized with metric_name={self.metric_name},",
    ...
    def _default_setting(self):
        ...
        ), "default setting only support single metric name"
        ...
        if check_order := _default_dict.get(metric_name):
            if check_order is None:
                raise ValueError(
                    f"No default setting for metric {metric_name}, "
                    + "you should provide @check_order manually"
                )
            else:
                return check_order

    def __call__(self, val_metrics: dict[str, float], *_discarded_args):
        ...
        if self.avg_metrics:
            for m in self.metric_name:
                assert (
                    m in val_metrics.keys()
                ), f"@val_metrics should have key {m} but got {val_metrics}"
        else:
            assert (
                self.metric_name in val_metrics.keys()
            ), f"@val_metrics should have key {self.metric_name} but got {val_metrics}"

        if not self.avg_metrics:
            new_val = val_metrics[self.metric_name]
        else:
            new_val = np.sum(
                [val_metrics[k] * self.avg_metrics_weights[k] for k in self.metric_name]
            )
        ...
        if self.verbose:
            if _save:
                logger.info(
                    f"best metric {self.metric_name} (weighted or one item), \n\t\t"
                    + f"[r]updated from {prev_val: .4f} to {new_val: .4f}[/r]"
                )
            else:
                logger.info(
                    f"best metric {self.metric_name} (weighted or one item), \n\t\t"
                    + f"[r]not updated, keep the best value {prev_val: .4f}[/r]"
                )
    ...
    def __repr__(self):
        return f"BestMetricSaveChecker(metric_name={self.metric_name}, check_order={self.check_order})"
...
class SavedWeightsNumMonitor:
    ...
    def rm_for_keep_k(self):
        if self.saved_num > self.kept_num:
            ...
            sort_fn = (
                self.sort_saved_fn
                if self.sort_saved_fn is not None
                else partial(sorted, key=lambda x: int(x))
            )
...
if __name__ == "__main__":
    ...
    val_d1 = {"sam": 2.3, "psnr": 10, "ssim": 0.8, "val_loss": 0.1}
    val_d2 = {"sam": 2.4, "psnr": 12, "ssim": 0.9, "val_loss": 0.2}
