"""
EMA model for PyTorch models.

Author: https://github.com/lucidrains, Zihan Cao
Date: 2025-02-04
...
logger = logging.getLogger("utils.EMA")
...
def is_model_parallel_initialized(model):
    return accelerate.PartialState().use_distributed
...
class EMA(Module):
    ...
    def __init__(
        ...
        state_include_online_model: bool = False,
    ):
        super().__init__()
        self.beta = beta
        ...
        self.state_include_online_model = (
            state_include_online_model and include_online_model
        )
    ...
    def state_dict(self):
        if self.state_include_online_model:
            if is_model_parallel_initialized(self.online_model):
                ema_model_state_dict = extract_model_from_parallel(
                    self.ema_model
                ).state_dict()
                online_model_state_dict = extract_model_from_parallel(
                    self.online_model
                ).state_dict()
                return {
                    "ema_model": ema_model_state_dict,
            ...
            if torch.distributed.is_initialized() or isinstance(
                self.online_model, torch.nn.parallel.DistributedDataParallel
            ):
                return extract_model_from_parallel(self.ema_model).state_dict()
            return self.ema_model.state_dict()

    def load_state_dict(self, state_dict, strict=True):
        # saved online_model and ema_model
        logger.info("load ema_model state dict")
        ...
        if (
            ...
            if is_model_parallel_initialized(self.online_model):
                self.ema_model.module.load_state_dict(
                    state_dict["ema_model"], strict=strict
                )
                ret = self.online_model.module.load_state_dict(
                    state_dict["online_model"], strict=strict
                )
            else:
                self.online_model.load_state_dict(
                    state_dict["online_model"], strict=strict
                )
                ret = self.ema_model.load_state_dict(
                    state_dict["ema_model"], strict=strict
                )
            logger.info("load online_model and ema_model state dict")
        # only has ema_model
        else:
            if is_model_parallel_initialized(self.online_model):
                ret = self.ema_model.module.load_state_dict(state_dict, strict=strict)
            else:
                ret = self.ema_model.load_state_dict(state_dict, strict=strict)
            logger.warning(
    ...
    def _zero_grad(self, set_to_none=False):
        for n, p in self.get_params_iter(self.ema_model):
        ...
        for n, p in self.get_params_iter(self.online_model):
            if p.grad is not None:
                if set_to_none:
                    p.grad = None
                else:
...
class PickleableEMA(nn.Module):
    def __init__(self, *modules):
        super(PickleableEMA, self).__init__()
