...
class DeepspeedEMA:
    def __init__(
        ...
        with (
            GatheredParameters(list(parameters)) and torch.no_grad()
        ):  # no need to register to extern parameters
            for p in parameters:
                if p.requires_grad:
                    # weakref still work?
    ...
    def copy_to(self, parameters: Optional[Iterable[torch.nn.Parameter]] = None):
        """
    ...
    def store(self, parameters: Optional[Iterable[torch.nn.Parameter]] = None):
        """
...
from accelerate import Accelerator, PartialState, DistributedType
