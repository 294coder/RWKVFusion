...
def EN_torch(img: torch.Tensor):
    a = img.flatten().round().type(torch.uint8)
    h = torch.bincount(a, minlength=256) / a.shape[0]
...
def MI_torch(image_F, image_A, image_B):
    ...
    image_A = image_A.int()
    image_B = image_B.int()
    return mutual_info_score(image_F.flatten(), image_A.flatten()) + mutual_info_score(
        image_F.flatten(), image_B.flatten()
    )
...
def MSE_torch(image_F, image_A, image_B):
    return (
        torch.mean((image_A - image_F) ** 2) + torch.mean((image_B - image_F) ** 2)
    ) / 2
...
def CC_torch(image_F, image_A, image_B):
    rAF = torch.sum(
        (image_A - torch.mean(image_A)) * (image_F - torch.mean(image_F))
    ) / torch.sqrt(
        (torch.sum((image_A - torch.mean(image_A)) ** 2))
        * (torch.sum((image_F - torch.mean(image_F)) ** 2))
    )
    rBF = torch.sum(
        (image_B - torch.mean(image_B)) * (image_F - torch.mean(image_F))
    ) / torch.sqrt(
...
def PSNR_torch(image_F, image_A, image_B):
    return 10 * torch.log10(
        torch.max(image_F) ** 2 / MSE_torch(image_F, image_A, image_B)
    )
...
def SCD_torch(image_F, image_A, image_B):
    ...
    imgF_B = image_F - image_B
    corr1 = torch.sum(
        (image_A - torch.mean(image_A)) * (imgF_B - torch.mean(imgF_B))
    ) / torch.sqrt(
        (torch.sum((image_A - torch.mean(image_A)) ** 2))
        * (torch.sum((imgF_B - torch.mean(imgF_B)) ** 2))
    )
    corr2 = torch.sum(
        (image_B - torch.mean(image_B)) * (imgF_A - torch.mean(imgF_A))
    ) / torch.sqrt(
        (torch.sum((image_B - torch.mean(image_B)) ** 2))
...
def VIFF_torch(image_F, image_A, image_B):
    ...
    return visual_information_fidelity(image_A, image_F) + visual_information_fidelity(
        image_B, image_F
    )
...
def Qabf_getArray_torch(img: torch.Tensor):
    ...
    # Sobel Operator Sobel
    h1 = torch.tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]], dtype=torch.float32).to(
        device
    )
    h2 = torch.tensor([[0, 1, 2], [-1, 0, 1], [-2, -1, 0]], dtype=torch.float32).to(
        device
    )
    h3 = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=torch.float32).to(
        device
    )

    SAx: torch.Tensor = torch.nn.functional.conv2d(
        img, h3.unsqueeze(0).unsqueeze(0), padding=1
    )[0, 0]
    SAy: torch.Tensor = torch.nn.functional.conv2d(
        img, h1.unsqueeze(0).unsqueeze(0), padding=1
    )[0, 0]
    gA = torch.sqrt(torch.mul(SAx, SAx) + torch.mul(SAy, SAy))
    aA = torch.zeros_like(img[0, 0])

    # * to suit vmap batching process
    # original code
    # aA[SAx == 0] = math.pi / 2
    # aA[SAx != 0] = torch.atan(SAy[SAx != 0] / SAx[SAx != 0])

    # modified code
    aA = torch.where(SAx == 0, torch.tensor(math.pi / 2), aA)
    aA = torch.where(
        SAx == 0,
        torch.tensor(math.pi / 2, device=SAx.device, dtype=SAx.dtype),
        torch.atan(
            torch.where(
                SAx != 0,
                SAy / SAx,
                torch.tensor(0.0, device=SAx.device, dtype=SAx.dtype),
            )
        ),
    )
...
def Qabf_getQabf_torch(aA, gA, aF, gF):
    ...
    GAF = torch.where(
        gA > gF,
        gF / gA,
        torch.where(gA == gF, gF, torch.where(gA < gF, gA / gF, torch.zeros_like(gA))),
    )
...
def evaluate_VIF_metric_numpy(
    image_vis, image_ir, image_f, metrics: "list[str] | str" = "all"
) -> dict:
...
if __name__ == "__main__":
    ...
    fused = read_image(
        "/Data3/cao/ZiHanCao/exps/panformer/visualized_img/panRWKV_v8_cond_norm/msrs_v2/00004N.png"
    )[None].float()
    ir = read_image("/Data3/cao/ZiHanCao/datasets/MSRS/test/ir/00004N.jpg")[
        None
    ].float()
    vi = read_image("/Data3/cao/ZiHanCao/datasets/MSRS/test/vi/00004N.jpg")[
        None
    ].float()
    ...
    print(timeit(lambda: evaluate_VIF_metric_torch(fused, ir, vi, "all"), number=10))
    print(
        timeit(
            lambda: evaluate_VIF_metric_numpy(
                fused.cpu().numpy(), ir.cpu().numpy(), vi.cpu().numpy(), "all"
            ),
            number=10,
        )
    )
