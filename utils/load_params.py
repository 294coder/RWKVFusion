...
def module_load(
    ...
    if path.endswith("pth") or path.endswith("pt") or path.endswith("bin"):
        ...
        load_engine = lambda weight_path, map_location: OrderedDict(
            load_file(weight_path, device=map_location)
        )
    else:
        assert osp.isdir(
            path
        ), f"{path} should be a directory to load deepspeed checkpoint"
        load_engine = lambda path, **_kwargs: load_state_dict_from_zero_checkpoint(
            model, path
        )
    ...
    # deprecated
        ...
        logger.print(
            ">>> did not find the pth file, try to find in used_weights/ and ununsed_weights/...",
            logging.INFO,
        )
        try:
    ...
    if spec_key is not None:
        ...
        try:
            ...
            logger.warning(
                f">>> not found parsed model `{spec_key}`, load the model directly \n \n"
            )
    ...
    if not strict:
        logger.warning(
            "model load strict is False, set it to True if you know what you are doing"
        )
    ...
    def _iter_params_load_fn(model, params_load, strict):
        ...
        if not isinstance(params_load, (list, tuple)):
            param_load_ziped = (
                list(params_load.items())
                if isinstance(params_load, dict)
                else params_load
            )
            for (s_name, s_param), (name, param) in zip(
                param_load_ziped, model.named_parameters()
            ):
                ...
                if saved_shape != required_shape:
                    if strict:
                        ...
                        if not full_unmatched_log:
                            logger.error(
                                "model load failed! shape of params does not match!"
                            )
                            raise RuntimeError(
                                "model load failed! shape of params does not match!"
                            )
                        ...
                        logger.info(
                            f"skip the shape mismatched param, param name {name}, ",
            ...
            for s_param, param in zip(params_load, model.parameters()):
                ...
                if saved_shape != required_shape:
                    if strict:
                        ...
                        logger.warning(
                            f"skip the shape mismatched param, current shape {required_shape} but loaded shape {saved_shape}"
                        )
                        continue
                param.data.copy_(s_param.data)

    def _load_fn(model, params_load, strict):
        if isinstance(params_load, (OrderedDict, dict)):  # ordered dict
