...
logger = easy_logger(func_name="MFFWDataset")


class MFFWDataset(Dataset):
    def __init__(
        self,
        data_dir: str,
        get_name: bool = True,
        ...
        self.far_path = list((self.data_dir / "mffwB").glob("*.*"))
        self.near_path = list((self.data_dir / "mffwA").glob("*.*"))
        if with_mask:
        ...
        if with_txt:
            logger.info(
                f'load t5 feature from {self.data_dir / "t5_feature_mffw.safetensors"}'
            )
            self.txt_feature = load_file(self.data_dir / "t5_feature_mffw.safetensors")
    ...
    def __getitem__(self, index):
        far, near = (
            self.far_images[index] / 255.0,
            self.near_image[index] / 255.0,
        )
...
if __name__ == "__main__":
    dataset = MFFWDataset(
        "/HardDisk/ZiHanCao/datasets/MFF-MFFW", with_txt=True, with_mask=True
    )
